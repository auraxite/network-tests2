...
std::vector<double> run_one_to_one(int rank, const Task &t, const Args &args,
	...
	/* Заполняем содержимое исходящего буфера, чтобы не гнать неинициализированные
	   страницы. Делаем КАЖДЫЙ запуск пары, потому что сам буфер переиспользуется
	   между парами и его содержимое могло быть перетёрто (или быть мусором при
	   первой же паре после cudaMalloc). */
	if (is_sender) {
