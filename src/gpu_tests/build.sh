...
# Без аргументов скрипт перезапускает сам себя на узле с GPU (на логин-узле
# обычно нет CUDA-заголовков). Аргумент build — это уже сама сборка.
if [ "${1:-srun}" = srun ]; then
	export OMPI_PATH UCX_PATH CUDA_PATH
	# --gres=gpu:1 вместо --gpus=1: работает на любом Slurm с настроенным GRES,
	# тогда как --gpus требует TRES-трекинга GPU, включённого не везде.
	...
	echo "  build:          собрать прямо здесь (мы уже на нужном узле)" >&2
	exit 1
fi
...
for tool in "$MPICXX" "$NVCC"; do
	test -x "$tool" || { echo "build.sh: not executable: $tool" >&2; exit 1; }
done
